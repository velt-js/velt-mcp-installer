 ...
 * Assumes add-velt-next-js CLI has already been run.
 *
 * @module integration
 */
...
function ensureCliOutputExists(structure, validationIssues) {
  ...
  for (const file of requiredFiles) {
    if (!fs.existsSync(file.path)) {
      validationIssues.push({
        file: file.name,
        type: 'missing-file',
        message: `Expected file generated by add-velt-next-js CLI is missing: ${file.name}`,
      });
      missingCount++;
    }
  }
  ...
  if (missingCount >= requiredFiles.length / 2) {
    validationIssues.push({
      type: 'cli-not-run',
      message: 'CLI output not found. Please run `add-velt-next-js` first before running integration.',
    });
    return false;
  }
